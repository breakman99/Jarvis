...
| 组件 | 文件 | 职责 |
|------|------|------|
| AgentRoleConfig | `src/agent/role_config.py` | 角色名、提示词、工具子集、策略类型 |
| BaseAgent / ConfigurableAgent | `src/agent/base_agent.py` | `plan -> execute -> response` 模板方法 |
| PlannerProtocol | `src/agent/planner.py` | 规划策略接口（`LLMPlanner` / `NullPlanner`） |
| ExecutorProtocol | `src/agent/agent_executor.py` | 执行策略接口（`LoopExecutor`） |
| AgentFactory | `src/agent/factory.py` | 根据角色配置创建 Agent 实例 |
| AgentRouter | `src/agent/router.py` | 按输入/上下文选择目标 Agent |
| AgentCoordinator | `src/agent/coordinator.py` | 管理 memory、route、session 与调度 |
