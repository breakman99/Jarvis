...
- **Agent 编排**：`AgentApp` 统一装配 `AgentCoordinator`，由 `PlanningAgent` + `ConversationAgent(Orchestrator)` 协作执行，`AgentSession` / `AgentResponse` 管理会话与输出。
...
│   ├── DESIGN_MULTI_AGENT.md  # 多 Agent 编排（Coordinator / PlanningAgent / ConversationAgent）
